...
keywords: [JavaScript, 變數, 作用域, variable, scope, let, const, var, block scope]
slug: variable-declare
---

**宣告** (declaration) 是告訴電腦需要**一個記憶體空間**來存放資料，類似**置物櫃**的概念。

## let / const

- `let` 可以隨時賦值 (assign) 改變，宣告時可以不賦值，內容為 `undefined`。
- `const` 指不會變動的內容 (constant)，宣告後**必須賦予初始值，且不能改變內容**。
...
`let` 或 `const` 的作用區間以**大括號**為限，大括號以外就無法存取變數，迴圈、函式，或任一組大括號等都符合這個規則：
...
大部分的語言也遵循這種以大括號為限來設計的，但是 `var` 的行為不同：
...
迴圈跑完後可以查看 `a` 的值是 `11`，這是正確的，但是連 `i` 的值都可以存取！

這是因為 `var` 真正的作用域是在**函式內部**，而 `for` 只是多個大括號的連續執行，並不是函式。
...
所以 `setTimeout` 給予時間參數 `0` ，代表它是**立刻進入排程空間**，不是馬上執行，所以主程序的運算繼續執行，到下一個循環 `setTimeout` 的內容：
...
`foo` 執行完後仍然可以找到 `a`，此時 `a` 是全域物件 **window** 的一個**屬性** (property)，用 `window.a` 可以取出來，所以 `a` 目前不是一個獨立的變數。
