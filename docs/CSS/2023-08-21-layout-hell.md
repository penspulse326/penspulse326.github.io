...
我選用的教材是 Huli 大大開設的 Lidemy 網站課程，  
以及參考他自辦 bootcamp 的課綱，按表操課地自學，只是沒有老師批改......

所以經歷六角學院在 2022 年末舉辦的精神時光屋活動後， 打擊對我來說有點大，  
每週主題我都嘗試用 React 去製作，想藉此熟悉 React，  
但沒有使用過其他可以搭配的 CSS 框架，僅使用在課綱內學過的 styled-component，  
所以幾乎是以純手刻的方式做開發。

因為沒有前期規劃的概念，導致連續三週的挑戰都沒有時限內完成，  
經常需要邊改樣式邊整合 React 邏輯，既沒有效率，最終作品的呈現也不盡人意，  
等於版面與功能我都沒有兼顧好。
...
大部分設計稿會依循網格系統做出來，所以網頁內許多元素的排列其實是有規律的，  
但沒有這些前備知識的話， 拿到設計稿可能會毫無頭緒從何開始，  
比如各個區塊的大小、間距、RWD 變化方式等等。

或是開 class 後寫了一卡車屬性，但不知從何整理，  
常常想到什麼就寫什麼，最後發現明明是相似的結構，  
卻重複寫了好幾個差不多的 class...

如果能以網格系統的概念拆解設計稿的各個區塊，  
實作過程就能具備規律，後續修改也比較容易，  
如果設計稿本身是沒有網格系統的，也允許我們做編排的微調，  
也可以自己加入，方便開發上的管理。
...
能夠嘗試寫出簡單的網格系統後，對框架的運用也會更有概念，  
我自己使用的這套邏輯也是因為大學上課曾經使用過 Bootstrap 而參考它做出來的 XD  
但手刻過一遍再運用到切版作業上，本來對 Bootstrap 的一些設定似懂非懂，  
也變得比較清楚網格系統是如何運用的。
...
到下面的圖卡、文字等，它們距離螢幕左右的間距都是一樣的，  
這是因為這些元素都放在**容器**裡面！

![PCHOME首頁](https://drive.google.com/uc?export=view&id=18KIJ5iOfRKxx3OR0rS9hWDVlB7Y5v5hw)

在遇到螢幕（解析度）較寬的情況下，  
設計上會希望內容不會因為螢幕的寬度變寬，導致圖片或區塊的編排也被橫向拉扯，  
這時會設定一個容器的最大寬度來保證元素不會超出這個範圍。

如設計稿上桌機開版為 1920px、平板 768px、手機 375px，  
容器的最大寬度可能就會設為：1296px、696px、348px。  
...
斷點的寫法就因人而異，這邊 1400px 才開始呈現設計稿所畫的桌機版，  
不過原則都是一樣的：

1. 如果能用最少的斷點作出所有版型是最好的
2. 無論怎麼切最後都要在設計稿指定的解析度上呈現一樣的結果

不然 UI 會追著你跑~~問你切的這是三小~～

---

## 格子 Column

決定好容器的寬度後，容器內會依版型分成 12 格～ 4 格等等，  
手機通常是 4 格、桌機 12 格，平板就比較不一定，各種格數都有人設計過。

採用網格系統設計的稿件，裡面的元素寬度大多也會依循格子數量做設定或排列， 
...
某些常見的數字大多是 Bootstrap、Tailwind 這些框架的斷點，方便前端直接套用開發。  
如設計稿沒有給定，~~我們就要自己通靈~~，可以跟設計師討論有沒有運用網格的必要，  
或是使用框架先快速 mock 一版簡單的出來再跟團隊討論要不要調整。

例如在 1920 的版面下，大部分元素都安排在距離螢幕邊界 400px 的地方，  
我們就可以設定最大寬度為 1920 - 400\*2 = 1120px。

由於裡面的格子是平均分散而不是緊緊相連的，  
所以稿件通常也會設定好每一格的間距，稱為 gutter，    
...
1. 用 for 迴圈來產生一連串的 class 名稱
2. width 這個 css 屬性可以透過 calc() 做百分比和 px 等等各種單位的混合計算
3. 100% 代表容器的寬度
4. 所以分散成 12 格並不是直接用 100% 除以 12，要先扣掉格子與格子的間距
5. 這個間距就是 gutter
6. ~~看不懂直接抄沒關係~~

以桌機為例會有 12 個格子，因此有 11 個間距，
扣掉全部間距佔用的寬度 $gutter \* 11，再除以 12，  
就是目前這個容器的寬度下每個格子真正的寬度，  
...
我們在 CSS 裡面就不要再為這個元素設與寬度有關的屬性，  
否則會因為複寫的關係，使 col 計算好的寬度失效。

因為容器是以 max-width 定義的，所以畫面寬度在伸縮時，  
容器的寬度也會不斷改變，而我們剛剛在計算格子寬度時是以容器的 100% 為基準，  
所以容器在拉伸時，這些 class 有下 col 的元素也會等比例拉伸。
...
熟悉 Bootstrap 的人應該知道，擁有格子 class 的元素都要放在 row 裡面：
...
（因為前面設定手機是 4 格，4 格就是滿版了）

我們設定了 wrap，以及用 col-XX 控制每個元素的寬度，  
所以在寬度低於桌機的斷點時，這些元素會變成滿版而像是單行排列的樣子。

![兩個版型的 row 變化](https://drive.google.com/uc?export=view&id=1R_Gq21EPNuZZ1fqxJu1E9ULFewBMVHRp)

這時 row 或是其他具有 flex 屬性的容器，搭配 gap 屬性就可以起到很好的分隔效果，  
這些元素會依照 gap 的數值分開，並且頭尾元素也不會產生額外的間距而要另外修改。
...
在我擁有網格的概念後，最近做作業都會先依循這個方法設定 SCSS，  
再開始做切版的準備，當然切版還有很多美美嘎嘎和要琢磨的地方，  
